...
std::tuple<float, float> SoftBoxCluster::addCluster(const edm4hep::Cluster& cluster, float Bz, bool usePt) {
	float val = 0;
	if (usePt) { val = cluster.getEnergy() * TMath::Cos(cluster.getITheta(); }
	else { val = cluster.getEnergy(); }
	
	clusterValPairs.push_back(std::make_pair(cluster, val));
...
std::vector<std::pair<const edm4hep::Cluster, float>> SoftBoxCluster::getClusters() const { return clusterValPairs; }
